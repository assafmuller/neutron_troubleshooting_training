...
for i in {0..2}
do
ssh heat-admin@overcloud-controller-$i 'sudo crudini --set /etc/neutron/l3_agent.ini DEFAULT interface_driver neutron.agent.linux.interface.OVSInterfaceDriver'
ssh heat-admin@overcloud-controller-$i 'sudo systemctl start neutron-l3-agent'
done
...
neutron router-gateway-clear exercise_04

neutron router-interface-delete exercise_04 exercise_04
neutron router-delete exercise_04
subnet_id=`neutron subnet-show -F id --format=value exercise_04`
...
neutron net-delete exercise_04
